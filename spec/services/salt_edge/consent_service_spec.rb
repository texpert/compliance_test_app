...
RSpec.describe SaltEdge::ConsentService do
  ...
  describe '#create_consent' do
    ...
    it 'creates a consent through the HTTP adapter and maps the response' do
      ...
      expect(a_request(:post, 'https://priora.saltedge.com/artea_sandbox/api/berlingroup/v1/consents')).to have_been_made
      expect(
        a_request(:post, 'https://priora.saltedge.com/artea_sandbox/api/berlingroup/v1/consents').with(
          headers: {
            'TPP-Redirect-Preferred' => 'true',
            'TPP-Redirect-URI' => 'https://example.ngrok.io/callback/123',
      ...
      expect(signer).to have_received(:build_headers).with(
        method: 'post',
        path: '/artea_sandbox/api/berlingroup/v1/consents',
        body: '{"access":{"allPsd2":"allAccounts"},"recurringIndicator":true,"validUntil":"2026-12-31","frequencyPerDay":4,"combinedServiceIndicator":false}',
        additional_headers: { 'TPP-Redirect-URI' => 'https://example.ngrok.io/callback/123' }
      )
    end

    it 'raises when upstream returns a non-2xx response' do
      stub_request(:post, 'https://priora.saltedge.com/artea_sandbox/api/berlingroup/v1/consents')
  ...
  describe '#consent_status' do
    ...
    it 'calls the status endpoint and returns consentStatus' do
      ...
      expect(a_request(:get, 'https://priora.saltedge.com/artea_sandbox/api/berlingroup/v1/consents/consent-123/status')).to have_been_made.once
      expect(result).to eq('valid')
    end
  end

  describe '#create_and_persist_consent' do
    let(:consent_record) { provider.consents.create!(upstream_consent_id: nil, status: Consent::STATUS_PENDING, callback_params: {}) }
    ...
    it 'sends the request, persists upstream ids, records a success event, and returns response' do
      stub_create_consent(consent_id: 'consent-xyz', consent_status: 'partiallyAuthorised',
                          sca_url: 'https://aspsp.example/sca')
      ...
      event = consent_record.events.where(event_type: 'consent_create').last
      expect(event).to be_present
      expect(event.request_body).to include('access' => { 'allPsd2' => 'allAccounts' })
      expect(event.request_headers).to include('Signature' => 'sig')
      expect(event.response_body['consentStatus']).to eq('partiallyAuthorised')
    end

    it 'records a failure event and raises when upstream request fails' do
      stub_request(:post, 'https://priora.saltedge.com/artea_sandbox/api/berlingroup/v1/consents')
        .to_return(status: 400, body: '{"tppMessages":[{"text":"upstream failed"}]}')

      expect do
        service.create_and_persist_consent(consent: consent_record)
      end.to raise_error(SaltEdge::RequestError, 'upstream failed')

      expect(consent_record.reload.upstream_consent_id).to be_nil

      event = consent_record.events.where(event_type: 'consent_create').last
      expect(event).to be_present
      expect(event.response_body['error']).to eq('upstream failed')
    end

    it 'raises when saving the consent to DB fails and does not persist upstream id' do
      stub_create_consent(consent_id: 'consent-xyz', consent_status: 'received',
                          sca_url: 'https://aspsp.example/sca')
