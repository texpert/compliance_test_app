...
|---|---|---|---|---|---|---|
| 4.1 Create consent | `POST /v1/consents` | POST | `X-Request-ID`, `Date`, `Content-Type`, `Digest`, `Signature`, `TPP-Signature-Certificate`, `TPP-Redirect-URI`, `TPP-Nok-Redirect-URI`, `PSU-IP-Address` | `{"access":{"allPsd2":"allAccounts"},"recurringIndicator":true,"validUntil":"2099-12-31","frequencyPerDay":4,"combinedServiceIndicator":false}` | HTTP 201 `{"consentId":"…","consentStatus":"received","_links":{"scaRedirect":{"href":"…"},"self":{"href":"…"},"status":{"href":"…"}}}` | ⚠️ verify base URL in portal |
...
| 4.3 Pass SCA | ASPSP sandbox SCA screen | n/a (PSU action in browser) | n/a | n/a (sandbox: auto-approve or test credentials) | ASPSP redirects PSU to `TPP-Redirect-URI?state={state}` | ⚠️ verify auto-approve vs credentials in portal |
| 4.4 Callback handling | `GET /callback?state={state}` | Rails inbound | n/a (inbound from ASPSP) | n/a | App validates `state`, calls `GET /v1/consents/{consentId}/status`, confirms `consentStatus: valid` | ⚠️ verify exact callback query params |
