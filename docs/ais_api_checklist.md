...
| 4.7 Transactions list | `GET /artea_sandbox/api/berlingroup/v1/accounts/{account-id}/transactions?bookingStatus=both&dateFrom=YYYY-MM-DD&dateTo=YYYY-MM-DD[&paginated=1]` | GET | `X-Request-ID`, `Date`, `Digest`, `Signature`, `TPP-Signature-Certificate`, `Consent-ID` | none | HTTP 200 `{"transactions":{"booked":[…],"pending":[…]}[,"_links":{"next":{"href":"…"},"account":{"href":"…"}}]}` — `_links.next` present only when paginated=1 and more pages exist | ✅ confirmed 2026-04-21; `paginated` accepts `1`/`0` not `true`/`false` |
