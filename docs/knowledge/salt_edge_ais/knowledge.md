...
#### Transaction fields (from `GET /accounts/{account-id}/transactions` response)
| Upstream field | Type | Notes |
|---|---|---|
| `transactionId` | string | May be absent for pending transactions |
| `bookingDate` | date string | `YYYY-MM-DD`; when the transaction posted |
| `valueDate` | date string | `YYYY-MM-DD`; value date |
| `transactionAmount.amount` | decimal string | Signed (negative = debit) |
| `transactionAmount.currency` | string | ISO 4217; falls back to account currency if absent |
| `creditorName` | string | Payee name (outgoing transactions) |
| `creditorAccount.iban` | string | Payee IBAN |
| `debtorName` | string | Payer name (incoming transactions) |
| `debtorAccount.iban` | string | Payer IBAN |
| `remittanceInformationUnstructured` | string | Free-text payment reference |
| `proprietaryBankTransactionCode` | string | Bank-specific transaction code |

Response structure groups transactions by booking status:
```json
{ "transactions": { "booked": [...], "pending": [...] } }
```

- `booked`: settled transactions; upsert key `(account_id, transaction_id)` — partial unique index WHERE `transaction_id IS NOT NULL`.
- `pending`: unsettled; `transactionId` often absent. Deleted and recreated on each fetch (no stable upsert key).

#### Transactions Pagination (confirmed against Artea sandbox, 2026-04-21)
- `paginated` query param accepts `1` or `0` — **not** `true`/`false`; using `true` returns a validation error.
- Without `paginated=1`: all matching transactions returned in one response.
- With `paginated=1`: up to 50 transactions per page; `_links.next.href` present when more pages exist.
- `_links.next.href` format: `/{provider_code}/api/berlingroup/v1/accounts/{id}/transactions?bookingStatus=…&dateFrom=…&dateTo=…&limit=50&offset=N&paginated=1`
- Follow `_links.next.href` sequentially until the field is absent (last page).
- Persist each page before fetching the next to keep memory bounded to one page at a time.
